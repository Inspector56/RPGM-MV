        ...
        if (fs.existsSync(curr)) {
            ...
            files.forEach(function(file, index) {
                if (fs.existsSync(path.join(parentPath, file)) && fs.lstatSync(path.join(parentPath, file)).isDirectory()) {
                    ...
                    const targetFile = path.join(src, file);
                    const patchFile = path.join(this._path, this._patchesFolder, name, parentPath, file);
                    ...
                    //If file is exclusive to the mod/patch, copy it over
                        ...
                        AutoDiff.createReqDirectories(targetFile);
                        fs.writeFileSync(targetFile, fs.readFileSync(path.join(curr, file)));
                        //Map changed assets to different encryption keys; ("no encryption key" may be
                        //  a "new" encryption key; thus we do this even if the extension claims the file
                        //  is not encrypted)
                        if (file.match(/\.(png|rpgmvp)$/i)) {
...
ModManager.patchText = function(sourceFile, patchFile) {
    ...
    try {
        ...
        fs.writeFileSync(sourceFile, fileText, {encoding: 'utf8'})
        return true;
    } catch (except) {
        return false;
    }
...
ModManager.patchLine = function(source, patchDiff) {
    ...
    for (let i = 0; i < patchDiff.length; i++) {
        ...
        if (lineSpec[0] == '-') {
            ...
            deleteIndices.forEach(function(deleteIndex, i, dI) { dI[i] = deleteIndex - 1; } )
        ...
        if (lineSpec[0] == '+') {
            ...
            deleteIndices.forEach(function(deleteIndex, i, dI) { if (deleteIndex > lineSpec[1]) { dI[i] = deleteIndex + 1; } })
        }
    }

    this._modifiedLines = deleteIndices.concat(addIndices);
...
ModManager.patchJSON = function(sourceFile, patchFile) {
    try {
        ...
        //Overwrite file with patched contents
        fs.writeFileSync(sourceFile, JSON.stringify(source), {encoding: 'utf8'});
        return true;
    } catch (except) {
        return false;
    }
...
//    JSON.parse('{"bob":"ar' + "\\"+'G}'); TODO DELETE
...
ModManager.applyFunctionSafeties = function(file, patchname) {
    ...
    file.replace(define, function(match, funName, offset, string) {
        ...
        if (nav && typeof nav === 'function') {
            let origFunc = String(nav).replace(/(?:[\n\r])+/g, '\n').replace(/(?:[\t ])+/g, ' ');
            origFunc = origFunc.replace(/\/\*(.?\s?)*?\*\//g, '');
            origFunc = origFunc.replace(/\/\/(.*)/g, '');
            origFunc = origFunc.match(/(?:\s)*(?:function(?:\s)*(?:.*?)(?:\s)*\((?:.*?(?:,(?:\s)*?(?:.*?))*?)\)(?:\s)*\{|\(?(?:.*?(?:,(?:\s)*?(?:.*?))*?)\)?(?:\s)*=>(?:\s)*\{)(?<bodyText>(?:.|\s)*)\}/);
            if (origFunc && ((origFunc[1]) == bodyText) ) {
                ...
                if (fs.lstatSync(path.join(parentPath, file)).isDirectory()) {
                    ...
                    //File exists in modded directory but not base directory:
                          ...
                          if (!file.match(/\.(png|mp4|ogg|rpgmvp|rpgmvo|rpgmvm)$/i) || assetCompare) {
                            ...
                            AutoDiffLogger.reportOverwrite(path.join(curr, file), newfile);
                          }
                    }
                }
            }.bind(this));
...
console.log(ModManager.applyFunctionSafeties(
    fs.readFileSync(
        path.join(ModManager._path, ModManager._installsFolder, 'play','js', 'rpg_core.js'),
    {encoding:"utf8"}), "sandwich"));

//TODO - currently playing anything where DataManager._verison is not null fucks up the
//  background of the title screen, no idea why, it doesn't seem to be trying to load the image
//  from the patch directory, all ImageManager.clear()s are commented out...

//TODO - save convert doesn't work - doesn't load modded map. ACTUALLY - it seems it kind of does...
//  BUT, new EVENTS are actually a part of GAME MAP, not DataMAP, so... have to figure out what to do there
//  It may be complicated...
//  This may also be an issue with Game_Player, Game_Party, possibly others - but those things, which span
//  the entire playthrough  (unlike the individual map a player happened to save on) we could just say are
//  the responsibility of the manual convert script

//TODO - patched script scrubbing to remove redundant function overwrites STILL doesn't work, it's
//  missing some basic-case matches for some reason.
...
//Another weakness was discovered in the "modded script sanitization" approach: if a core ("rpg_") script
